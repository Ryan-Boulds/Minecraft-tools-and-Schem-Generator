...
def create_animator_gui(parent_frame, app):
    ...
    replace the contents with the real animation controls.
    """
    ...
    placeholder_label = tk.Label(
        parent_frame,
        text="Animation controls go here.",
        font=("Segoe UI", 10),
    )
...
def process_command(app, command):
    """
    Placeholder command processor for the 'Animate' sub-tab.

    Wired into CommandModifierGUI.process_command() in main.py so
    clipboard-triggered processing on this tab doesn't error out.
    Replace with real animation command generation.
    """
    logging.debug("Animate tab: process_command called (not yet implemented)")